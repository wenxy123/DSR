...
class UNet3D(nn.Module):
    ...
    def forward(self, x):
        noise = torch.randn_like(x) * (0.1 ** 0.5)  # Noise with the same shape as x
        x = x + noise  # Adding noise to the data
    ...
    def sample(self, x, sample_size=2, expand_dim=False):
        ...
        # Generate noise for each sample and repeat the input x
        x_samples = []
        ...
        x_rep = torch.cat(x_samples, dim=0)  # Shape: (batch_size * sample_size, 3, D, H, W)

        # Pass through the network
        y_samples = self.forward(x_rep)  # Shape: (batch_size * sample_size, out_dim)

        if not expand_dim:
            return y_samples
        else:
            # Reshape the output to include the sample dimension
            y_samples = y_samples.view(data_size, sample_size, 3, 16, 16, 16)  # Shape: (sample_size, batch_size, out_dim)
            y_samples = y_samples.permute(0,2,3,4,5,1)  # Shape: (batch_size, out_dim, sample_size)
            return y_samples
    ...
    def predict(self, x, target="mean", sample_size=5):
        data_size = x.size(0)  # Number of input samples
        samples = self.sample(x, sample_size=sample_size, expand_dim=True)  # Shape: (batch_size, out_dim, sample_size)
        ...
        for t in target:
            if t == "mean":
                results.append(samples.mean(dim=-1))  # Mean across the samples dimension
            else:
                if t == "median":
                    t = 0.5
                assert isinstance(t, float), "Target must be a float for quantile prediction."
                results.append(samples.quantile(t, dim=-1))  # Quantile along the samples dimension
...
def DSR_train(x, y, val_x, val_y,
               ...
               #device="cpu",
               device = torch.device('cuda:0'),standardize=True, verbose=True):
...
class DSR(object):
    ...
    def train(self, x, y, val_x, val_y, num_epochs=None, batch_size=None, lr=None, print_every_nepoch=1, print_times_per_epoch=1,verbose=False):
        ...
        # Evaluate performance on the validation data (on the original scale)
        self.model.eval()
        self.tr_loss = self.eval_loss(val_x, val_y, loss_type="energy", verbose=True)
